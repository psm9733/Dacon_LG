...
from network.backbone.resnext.stage import ResidualStage


def ResNextBody(in_tensor, n_block_per_stage, filter_per_stage, kernel_size_per_stage, strides_per_stage,
...
if __name__ == "__main__":
    ...
    model = ResNextBody(input_tensor, [1, 1, 1, 1], [[32, 32, 32], [64, 64, 64], [128, 128, 128], [256, 256, 256]],
                       [(3, 3), (3, 3), (3, 3), (3, 3)], [(2, 2), (2, 2), (2, 2), (2, 2)], [8, 8, 16, 16], "relu", weight_decay)
    model = tf.keras.Model(inputs=[input_tensor], outputs=model)
    model.summary()
    # model.save("ResNextBody.h5")
