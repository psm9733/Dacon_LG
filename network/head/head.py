...
def Classification_Head(in_tensor, num_classes, weight_decay, name):
    ...
    out_tensor = tf.keras.layers.Dense(units=num_classes, kernel_regularizer=kernel_regularizer, activation='softmax',
                                       name="Head_{}".format(name))(out_tensor)
    return out_tensor


def MultiScale_Segmentation_HEAD(in_tensor_list, activation, weight_decay, name):
    kernel_regularizer = tf.keras.regularizers.l2(weight_decay)
    out_tensor_list = []
    for index, in_tensor in enumerate(in_tensor_list):
        ...
        out_tensor = tf.keras.layers.Conv2D(filters=1, activation=activation, kernel_size=(1, 1), strides=(1, 1),
                                            kernel_regularizer=kernel_regularizer,
                                            name="Head_{}_{}".format(name, index))(out_tensor)
...
def MultiScale_Regression_HEAD(in_tensor_list, activation, num_classes, weight_decay, name):
    ...
    for index, in_tensor in enumerate(in_tensor_list):
        ...
        out_tensor = tf.keras.layers.Reshape((1, 1, -1))(out_tensor)
        out_tensor = tf.keras.layers.Conv2D(filters=out_tensor.shape[-1], activation=activation, kernel_size=(1, 1),
                                            strides=(1, 1),
                                            kernel_regularizer=kernel_regularizer)(out_tensor)
        out_tensor_list.append(out_tensor)
    out_tensor = tf.keras.layers.Concatenate()(out_tensor_list)
